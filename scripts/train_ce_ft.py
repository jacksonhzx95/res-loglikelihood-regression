...
def main_worker(gpu, opt, cfg):
    ...
    #     optimizer,T_0=150, T_mult=4, eta_min=1e-6
    # )
    ...
    for i in range(cfg.TRAIN.BEGIN_EPOCH, cfg.TRAIN.END_EPOCH):
        ...
        if (i + 1) % opt.snapshot == 0:
            # Save checkpoint
            # if opt.log: # torch.save(m.module.state_dict(), './exp/{}-{}/model_{}.pth'.format(
                ...
                if output_3d:
                    ...
                    if opt.log and err <= best_err:
                        ...
                        torch.save(m.module.state_dict(),
                                   './exp/{}-{}/best_model.pth'.format(opt.exp_id, cfg.FILE_NAME))

                    logger.info(f'##### Epoch {opt.epoch} | gt results: {err}/{best_err} #####')

                else:
                    overview, mean = validate_gt(m, opt, cfg, heatmap_to_coord)
