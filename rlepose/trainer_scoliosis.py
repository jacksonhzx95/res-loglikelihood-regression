...
from rlepose.utils.metric_mape import cal_mape
...
def validate(m, opt, cfg, heatmap_to_coord, batch_size=20, use_nms=False):
    ...
    if opt.rank == 0:
        ...
        # this part (evaluate map) need modify
...
def validate_gt(m, opt, cfg, heatmap_to_coord, batch_size=20):
    ...
    if opt.rank == 0:
        ...
        mse, mape = cal_mape(kpt_json_all, cfg.DATA_PRESET.get('IMAGE_SIZE'))

        return acc_val_avg, mse, mape  # res['AP']
    else:
        return 0, 0, 0
