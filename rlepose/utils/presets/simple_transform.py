...
class CETransform(object):
    ...
    def __call__(self, src, label):
        ...
        if random.random() > 0.5 and self.flip and self._train:
            ...
            assert src.shape[2] == 3
            src = src[:, ::-1, :]
            joints = flip_joints_wo_pair(joints, imgwidth)
            center[0] = imgwidth - center[0] - 1
...
class CETransform_beta(object):
    ...
    def __call__(self, src, label):
        ...
        if self._train:
            sf = self._scale_factor
            scale = scale * random.uniform(0.5, 1 + sf)
            # scale = scale * random.uniform(0.75, 1 + sf)
        else:
            scale = scale * 1.0
...
class ScoliosisTransform(object):
    """Generation of cropped input person and pose heatmaps from SimplePose.
